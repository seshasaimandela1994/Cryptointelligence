...
func main() {
	...
	fmt.Println("  ========================================")
	fmt.Println("  Running on http://localhost:8084")
	fmt.Println()
	fmt.Println("  GET /v1/cex/stats")
	fmt.Println("  GET /v1/cex/candidates?limit=50&min_score=0.75")
	fmt.Println("  GET /v1/cex/exchange/:name")
	fmt.Println("  GET /v1/cex/screen/:address")
	fmt.Println("  GET /v1/cex/top/:role  (deposit|hot|cold)")
	fmt.Println()
...
func cexStatsHandler(c *gin.Context) {
	var wallets, scored, highConf, labels, exchanges, transfers, withUSD int64
	var topScore float64
	var lastUpdated sql.NullTime

	cexDB.QueryRow(`
		SELECT
			(SELECT COUNT(*) FROM wallet_addresses WHERE chain_id=1),
			(SELECT COUNT(*) FROM wallet_counterparty_stats_30d),
			(SELECT COUNT(*) FROM v_wallet_role_candidates WHERE best_candidate_score >= 0.90),
			(SELECT COUNT(*) FROM exchange_wallet_labels WHERE is_active=TRUE),
			(SELECT COUNT(*) FROM exchange_entities),
			(SELECT COALESCE(MAX(best_candidate_score),0) FROM v_wallet_role_candidates),
			(SELECT COUNT(*) FROM ethereum_token_transfers),
			(SELECT COUNT(*) FROM ethereum_token_transfers WHERE usd_value_estimate IS NOT NULL),
			(SELECT MAX(last_computed_at) FROM wallet_features_ethereum)`).Scan(
		&wallets, &scored, &highConf, &labels, &exchanges,
		&topScore, &transfers, &withUSD, &lastUpdated)

	last := ""
	if lastUpdated.Valid {
		last = lastUpdated.Time.Format(time.RFC3339)
	}
	c.JSON(200, gin.H{
		"total_wallets":            wallets,
		"scored_wallets":           scored,
		"high_confidence_candidates": highConf,
		"labeled_exchange_wallets": labels,
		"total_exchanges":          exchanges,
		"top_candidate_score":      topScore,
		"transfers_indexed":        transfers,
		"transfers_with_usd":       withUSD,
		"last_updated":             last,
	})
}
...
func cexCandidatesHandler(c *gin.Context) {
	...
	query := `
		SELECT vrc.address, vrc.best_candidate_role,
		...
		FROM v_wallet_role_candidates vrc
...
func cexExchangeHandler(c *gin.Context) {
	...
	err := cexDB.QueryRow(`
		SELECT ee.exchange_id, ee.canonical_name, ee.legal_name,
		...
		GROUP BY ee.exchange_id, ee.canonical_name, ee.legal_name, ee.trust_tier,
			ee.status, ee.exchange_category, ee.primary_jurisdiction, ee.website_url,
			rp.regulatory_status, rp.aml_risk_level,
			rp.enforcement_history_flag, rp.proof_of_reserves_flag
		LIMIT 1`, name).Scan(
		&p.ExchangeID, &p.CanonicalName, &legal,
		&p.TrustTier, &p.Status, &cat, &juris, &website,
		&p.RegulatoryStatus, &p.AMLRisk,
		&p.Enforcement, &p.POR,
		&p.LabeledWallets, &avgConf)

	if err == sql.ErrNoRows {
		c.JSON(404, gin.H{"error": "Exchange not found: " + name})
		return
	}
	if err != nil {
		c.JSON(500, gin.H{"error": err.Error()})
	...
	if arows != nil {
		defer arows.Close()
		for arows.Next() {
			var a string; arows.Scan(&a)
			p.Aliases = append(p.Aliases, a)
		}
	}

	wrows, err := cexDB.Query(`
		SELECT wa.address, ewl.wallet_role::text, ewl.wallet_subrole::text,
			ewl.attribution_method::text, ewl.confidence_score,
			ewl.confidence_band::text, ewl.review_status::text,
			COALESCE(ewl.analyst_notes,''),
	...
	if err == nil && wrows != nil {
		...
		for wrows.Next() {
			...
			wrows.Scan(&w.Address, &w.Role, &w.Subrole, &w.Method,
				...
				&w.FirstBlock, &w.LastBlock,
				&w.DepositScore, &w.HotScore, &w.OutboundUSD)
...
func cexScreenHandler(c *gin.Context) {
	...
	err := cexDB.QueryRow(`
		SELECT ee.canonical_name, ewl.wallet_role::text, ewl.confidence_score
		FROM exchange_wallet_labels ewl
		JOIN wallet_addresses wa ON wa.wallet_id = ewl.wallet_id
	...
	if walletID > 0 {
		...
		if err == nil {
			...
			if cand.BestScore >= 0.90 {
				result.RiskSignals = append(result.RiskSignals, fmt.Sprintf("HIGH_CONFIDENCE:%.2f", cand.BestScore))
			}
			if cand.OutboundUSD > 1_000_000 {
				result.RiskSignals = append(result.RiskSignals, fmt.Sprintf("HIGH_OUTBOUND_USD:$%.0f", cand.OutboundUSD))
			}
			if cand.UniqueSenders > 100 {
				result.RiskSignals = append(result.RiskSignals, fmt.Sprintf("MANY_SENDERS:%d", cand.UniqueSenders))
			}
		}
	}
	c.JSON(200, result)
...
func cexTopByRoleHandler(c *gin.Context) {
	...
	rows, err := cexDB.Query(fmt.Sprintf(`
		SELECT vrc.address,
			ROUND(COALESCE(%s,0)::numeric,4),
			COALESCE(%s,0),
			COALESCE(wcs.unique_token_senders_30d,0),
			COALESCE(wcs.unique_token_receivers_30d,0),
